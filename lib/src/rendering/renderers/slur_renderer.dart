...
class SlurRenderer {
  ...
  Offset _calculateSlurEndpoint(
    ...
    if (above && stemUp) {
      ...
      clearanceReason =
          'Slur ACIMA + stem UP → ponta da haste (${stemHeight}SS + ${clearanceFromStem}SS)';
    } else if (!above && !stemUp) {
      // Slur ABAIXO + stem DOWN: ir até a PONTA da haste + margem
      yOffset = (stemHeight + clearanceFromStem) * staffSpace;
      clearanceReason =
          'Slur ABAIXO + stem DOWN → ponta da haste (${stemHeight}SS + ${clearanceFromStem}SS)';
    } else {
    ...
    return Offset(notePos.dx + xOffset, noteY + yOffset);
  }
